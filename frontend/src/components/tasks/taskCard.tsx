...
export function PendingTaskCard({ name, description, data }: cardTask) {
    ...
    const accpetMutation = useMutation<Response, Error, acceptedTask>({
    ...
    return (
        <>
            <div className="task">
                ...
                <p>{description}</p>
                {data.map((value) => (
                    <a href={taskDataAccess + value} download >{value }</a>
                ))}
                <button className="task-accept" onClick={() => {
                    accpetMutation.mutate({
                        color: localcolor,
                        task: name
                    } as acceptedTask)
                }}>
    ...
    return (
        <>
            <div className="task">
                ...
                <div>
                    <input style={{color : "blue"} } onChange={(event) => input_solution = event.target.value}></input>
                    <button onClick={() => solveMutation.mutate({
                        color: localcolor,
                        task: name,
